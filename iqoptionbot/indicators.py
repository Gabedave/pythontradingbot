...
class Indicators():
    ...
    def macd(self, fast_period: int = 12, slow_period: int = 26, column_name: str = 'macd') -> pd.DataFrame:
        ...
        #--------------------------------------------------
        #self._frame['sma12'] = self._price_groups['close'].transform(lambda x: x.rolling(period=fast_period).mean())
        modPrice = self._price_groups['close']
        for key,_ in modPrice:
            b = self._price_groups.get_group(key)['close'].rolling(fast_period).mean()
            modPrice.get_group(key).iloc[:fast_period] = b[:fast_period].values
            
        # Calculate the Slow Moving MACD.
        self._frame['macd_fast'] = modPrice.transform(
            lambda x: x.ewm(span = fast_period, min_periods = fast_period, adjust=False).mean()
        )
        #---------------------------------------------------
        #self._frame['sma26'] = self._price_groups['close'].transform(lambda x: x.rolling(period=slow_period).mean())
        modPrice = self._price_groups['close']
        for key,_ in modPrice:
            b = self._price_groups.get_group(key)['close'].rolling(slow_period).mean()
            modPrice.get_group(key).iloc[:slow_period] = b[:slow_period].values
            
        # Calculate the Slow Moving MACD.
        self._frame['macd_slow'] = modPrice.transform(
            lambda x: x.ewm(span = slow_period, min_periods = slow_period, adjust=False).mean()
        )
        #---------------------------------------------------
        ...
        self._frame['macd'] = self._price_groups['macd_diff'].transform(
            lambda x: x.ewm(span = 9).mean()
        )
        
        return self._frame 

    def chopiness_index(self, period: int = 14, column_name: str ='chopiness') -> pd.DataFrame:
        """Calculates the Choppiness Index.

        Arguments:
        ----
        period {int} -- The number of periods to use when calculating 
            the choppiness index. (default: {26})

        Returns:
        ----
        {pd.DataFrame} -- A Pandas data frame with the Choppiness Index included.
    ...
    def add_crossover(self, line1:str , line2:str, column_name: str = None) -> pd.DataFrame:
        """Adds crossover column to dataframe
        Arguments
        --------------
        line1 = first curve
        line2 = second curve
        count = how many candles behind to check(1 - one caldle, : - all candles)
        ...
        self._current_indicators[column_name] = {}
        self._current_indicators[column_name]['args'] = locals_data
        self._current_indicators[column_name]['func'] = self.add_crossover

        if not column_name:
            column_name = 'crossover'
        
        cross_df = self._price_groups
        columns=self._frame.columns
        a = pd.DataFrame(index=self._frame.index)
        for key,_ in cross_df:
            y=_.loc[:, line1]
            z=_.loc[:, line2]

            crossover = np.pad(np.diff(np.array(y > z).astype(int)),(1,0), 'constant', constant_values = (0,))
    ...
    def mass_index(self, period: int = 9, column_name: str = 'mass_index') -> pd.DataFrame:
        ...
        self._frame['mass_index'] = self._frame['mass_index_raw'].transform(
            lambda x: x.rolling(period=25).sum()
        )
